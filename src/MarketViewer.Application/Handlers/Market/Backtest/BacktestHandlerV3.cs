...
public class BacktestHandlerV3(
    ...
    ILogger<BacktestHandlerV3> logger) : IRequestHandler<BacktestRequestV3, OperationResult<BacktestResponseV3>>
{
    ...
    public async Task<OperationResult<BacktestResponseV3>> Handle(BacktestRequestV3 request, CancellationToken cancellationToken)
    {
        try
        {
            ...
            if (backtestRepository.CheckForBacktestHistory(CompressRequestDetails(request), out var record))
            {
                s3Entries = await backtestRepository.GetBacktestResultsFromS3(record);
                ...
                {
                    var missingEntries = await backtestRepository.GetBacktestResultsFromLambda(new BacktestRequestV3
                    {
                        Id = request.Id,
                        Start = request.Start,
            ...
            {
                entries = await backtestRepository.GetBacktestResultsFromLambda(request);
            }
            ...
            var response = new OperationResult<BacktestResponseV3>
            {
                ...
                Data = new BacktestResponseV3
                {
                    ...
                    Hold = new BacktestEntryStats
                    {
                        ...
                        WinRatio = holdWins.Any() ? holdWins.Count() / (float)(holdWins.Count() + holdLosses.Count()) : 0,
                    ...
                    High = new BacktestEntryStats
                    {
                        ...
                        WinRatio = highWins.Any() ? highWins.Count() / (float)(highWins.Count() + highLosses.Count()) : 0,
                    ...
                    Other = request.Exit.Other is null ? null : new BacktestEntryStats
                    {
                        ...
                        WinRatio = otherWins.Any() ? otherWins.Count() / (float)(otherWins.Count() + otherLosses.Count()) : 0,
